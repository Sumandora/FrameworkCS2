...
EntityHandle<BaseEntity> BaseEntity::get_raw_handle() const
{
	if (EntityIdentity* identity = entity(); identity) {
		...
		if (identity->entry_index == GameEntitySystem::INVALID_ENTITY_HANDLE) {
			entry_index = 0x7fff;
		} else {
			entry_index = identity->entry_index & GameEntitySystem::ENTITY_LIST_INDEX_MASK;
		}
		const std::uint32_t index = ((identity->entry_index >> GameEntitySystem::ENTITY_SERIAL_BITS_COUNT) - (identity->flags() & 1)) * 0x8000 | static_cast<uint32_t>(entry_index);
